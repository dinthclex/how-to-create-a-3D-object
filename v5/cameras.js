...
function drawCameras(){
	//câmera movida pelo player (vista de topo)
	...
	drawCamera2D(camera.position.z, camera.position.x, camera.rotation.y, 'top', info.moveView.bits);
	...
	drawCamera2D(z, x, ang, 'top', info.topView.bits);
	...
	drawCamera2D(z, y, ang, 'side', info.sideView.bits);
	
}

function drawCamera2D(x=0, y=0, ang=0, view='top', en=0b11111111111){
	/*    x: 
	      y: 
	    ang: 
	   view: 'top' ou 'side'. A única diferença é que se for 'side', em vez de SCREEN_WIDTH, utiliza SCREEN_HEIGHT nos cálculos.
	     en: habilita a visualização de partes do desenho, cada bit define uma parte do desenho
	...
	*/
	...
	ang1 = Math.atan2((view=='top'?SCREEN_WIDTH:SCREEN_HEIGHT)/2, FOCAL_LENGTH); // pega o half fov atual
	r1 = d/Math.cos(ang1);
	ang1 = pa - ang1;
	x1 = r1*Math.cos(ang1);
	y1 = r1*Math.sin(ang1);
	...
	ang2 = Math.atan2((view=='top'?SCREEN_WIDTH:SCREEN_HEIGHT)/2, FOCAL_LENGTH); // pega o half fov atual
	r2 = d/Math.cos(ang2);
	ang2 = pa + ang2;
	x2 = r2*Math.cos(ang2);
	y2 = r2*Math.sin(ang2);
	...
	ctx.strokeStyle = "Red";
	ctx.beginPath();
	ctx.arc(px, py, 20, pa-ang1, pa+ang1, false);
	ctx.stroke();
	}
}