...
function drawScene(){
	...
	//retângulo que representa a tela (screen)
	...
	for(let j=0; j<scene.objects.length; j+=1){
		...
		if(obj.active){
			drawObjectZXTopView(obj, camera.position.z, camera.position.x, camera.rotation.y, false,false,true);
			drawObjectZXTopView(obj, camera.topView.z, camera.topView.x, camera.topView.ang);
			drawObjectZYSideView(obj, camera.sideView.z, camera.sideView.y, camera.sideView.ang);
		}
...
function drawXY(obj){
	...
	for(let i=0; i < obj.vertices.length; i+=1){
		...
		gx=obj.x+p.x; gy=obj.y+p.y; gz=obj.z+p.z;
...
function drawObjectOnCanvasZXTopView(obj){
	let c = document.getElementById("background");
	let ctx = c.getContext("2d");
	let gx,gy,gz;
	let hli = obj.active ? obj.highlightIndex : -1; // highlight index
...
function drawObjectZYSideView(obj, sidz=800, sidy=600, sida=0, pnts=true,lins=true,tris=true){
	let c = document.getElementById("background");
	let ctx = c.getContext("2d");
	let gx,gy,gz;
...
function drawObjectZXTopView(obj, topz=SCREEN_WIDTH/2, topx=SCREEN_HEIGHT/2, topa=HALF_PI, pnts=true,lins=true,tris=true){
	let c = document.getElementById("background");
	let ctx = c.getContext("2d");
	let gx,gy,gz;
