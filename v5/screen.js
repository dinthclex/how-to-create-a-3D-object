...
function drawScene(){
	...
	for(let j=0; j<scene.objects.length; j+=1){
		...
		//desenha um único objeto da lista de objetos contidos na cena - Espaço do Mundo
...
function screenText(){
	...
	let str = [{text:"considerações sobre o ponto",           color:"Black"},
			   ...
			   {text:"teclado: i, t, v, w, a, s, d, seta esquerda e seta direita",             color:"Black"},
	...
	text("=", x, y,"Black", "bold 1em consolas");
	x+=15;
	text("FOCAL_LENGTH", x, y,"CornflowerBlue", "bold 1em consolas");
	//apontamento para as proporções
	// seta
	x += 140;
	y += -5;
	line(x,y-30, x+50,y-10,    "Black",2,    "filledArrow","solid","flat");
	// texo "proporções para x"
	x += 55;
	y += -3;
	text("proporções para x", x, y,"Black", "bold 1em consolas");
	...
	y += -5;
	line(x,y-30, x+50,y-10,    "Black",2,    "filledArrow","solid","flat");
	// texo "proporções para y"
	x += 55;
	y += -3;
	text("proporções para y", x, y,"Black", "bold 1em consolas");
	}
